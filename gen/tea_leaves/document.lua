...
function Document:process_and_publish_results()
   local tks, err_tks = self:get_tokens()
   local uri_str = Uri.tostring(self._uri)
   if #err_tks > 0 then
      self:publish_diagnostics(uri_str, imap(err_tks, function(t)
         return {
            range = {
   ...
   if #parse_errs > 0 then
      self:publish_diagnostics(uri_str, imap(parse_errs, function(e)
   ...
   self:publish_diagnostics(Uri.tostring(self._uri), diags)
end
